...
const postQr = async <TResponse, TBody extends object>(url: string, body: TBody): Promise<TResponse> => {
    const response = await fetch(`/qr-api${url}`, {
        method: 'POST',
...
export const createQrToken = (student: QrStudentPayload) =>
    postQr<QrTokenResponse, { student: QrStudentPayload }>('/qr/create', { student })

export const scanQrToken = (token: string) => postQr<QrScanResponse, { token: string }>('/qr/scan', { token })

export const markQrPoints = (token: string) => postQr<QrMarkResponse, { token: string }>('/points/mark', { token })
